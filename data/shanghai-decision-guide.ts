...
export type ShanghaiFillStep = {
  step: number;
  title: string;
  action: string; // 这一步该做什么（可执行）
  why: string; // 为什么（规则依据/风险）
};

export const shanghaiFillStrategy: ShanghaiFillStep[] = [
  {
    step: 1,
    title: "先把候选分成冲 / 稳 / 保三档",
    action:
      "用上方“按分数看现实区间”把够得着的院校专业组分成冲、稳、保三档。24 个志愿建议大致按冲 6–8 个、稳 8–10 个、保 6–8 个分配，最后必须留 3–4 个分数明显低于你的组兜底。",
    why:
      "平行志愿一次投档、投出去就不再检索后面的志愿。没有足够保底，一旦前面全部够不到就会滑档、掉到征求志愿甚至下一批。",
  },
  {
    step: 2,
    title: "把三档合并成从高到低的 1–24 有序表",
    action:
      "不要分三列平铺，要合并成一张从第 1 到第 24、投档线由高到低排列的总表：冲的放最前、稳的居中、保的压最后。每一格都填你真的愿意去的组。",
    why:
      "上海普通批是“分数优先、遵循志愿、一次投档”：系统从你的第 1 志愿开始检索，投进第一个够得着的组就停。顺序排错（把保底放前面）会浪费掉冲和稳的机会。",
  },
  {
    step: 3,
    title: "每个组内排好 4 个专业 + 决定是否服从调剂",
    action:
      "每个院校专业组里再填 4 个专业志愿（按你的偏好从高到低），并勾选“是否服从专业调剂”。强烈建议：只填那些“就算被调剂到组内任意一个专业也能接受”的组，并勾服从。",
    why:
      "调剂只在被投档的这个组内进行。组内 4 个专业都没录上、又不服从调剂，就会被退档；服从则可能被调到组内任何一个专业——所以组内有没有你完全不能接受的专业，决定了该不该勾。",
  },
  {
    step: 4,
    title: "想清楚“不勾服从”的后果再决定",
    action:
      "如果你的分数只是刚好压着某个组的投档线、又不勾服从调剂，等于把退档风险拉满。要么换一个你分数更有余量的组，要么勾服从并确认组内专业都能接受。",
    why:
      "退档不是“后悔按钮”：被投档进组后若因不服从调剂落选，本批次本轮就结束了，只能等征求志愿（且不一定有、通常还要降分），代价很大。",
  },
  {
    step: 5,
    title: "用组内构成对冲调剂风险，并留够缓冲带",
    action:
      "对每个勾了服从的组，先打开《招生专业目录》看清这个组到底捆了哪些专业；把“想冲的好专业和你绝对不想读的专业捆在同一组”的组排除掉。保底的几个组要分数明显有余量，做真正的缓冲带。",
    why:
      "用强势专业拉高组线、再靠服从调剂把人灌进冷门专业，是平行志愿最常见的坑。保底组若也压线，就起不到兜底作用。",
  },
];
